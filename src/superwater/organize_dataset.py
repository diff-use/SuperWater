...
def main():
    ...
    for original_pdb_id, truncated_pdb_id in tqdm(unique_pdb_id_dict.items(), desc="Copying and renaming PDB files"):
        ...
        # Placeholder water file (contents irrelevant; only its presence/name matters).
        # No .mol2 is needed anymore -- the loader reads waters from the .pdb.
        water_src = os.path.join(dummy_water_path, "_water.pdb")
        water_dest = os.path.join(dest_folder, f"{truncated_pdb_id}_water.pdb")
