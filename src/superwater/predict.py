...
def predict_one(name, src_path, out_dir, score_dir, conf_dir, device, cfg, overwrite,
    ...
    # Placeholder water file: predicted waters are sampled, so its contents are irrelevant
    # (only a {name}_water.pdb needs to exist for the loader). No .mol2 is needed anymore.
    shutil.copy(os.path.join(DUMMY_WATER_DIR, "_water.pdb"), os.path.join(complex_dir, f"{name}_water.pdb"))
