...
def train_epoch(model, loader, optimizer, args, mad_prediction):
    ...
    for batch_idx, data in enumerate(tqdm(loader, total=len(loader))):        
        ...
        try:
            ...
            if not torch.isfinite(confidence_loss):
                print(f"| WARNING: non-finite confidence loss ({confidence_loss.item()}) at batch {batch_idx}, skipping batch")
            ...
            if not torch.isfinite(grad_norm):
                print(f"| WARNING: non-finite confidence grad norm ({grad_norm}) at batch {batch_idx}, skipping optimizer step")
                optimizer.zero_grad(set_to_none=True)
                continue
    ...
    for data in tqdm(loader, total=len(loader)):
        try:
            ...
            if mad_prediction:
                ...
                confidence_loss = F.mse_loss(pred, norm_labels)
                if not torch.isfinite(confidence_loss):
                    print("| WARNING: non-finite confidence validation loss, skipping batch")
                    continue
                meter.add([confidence_loss.cpu().detach()])
            else:
                if isinstance(args.mad_classification_cutoff, list):
...
def train(args, model, optimizer, scheduler, train_loader, val_loader, run_dir):
    ...
    for epoch in range(args.n_epochs):
        ...
        if not metric_is_finite:
            print(f"| WARNING: non-finite validation metric {args.main_metric}={metric_value}; skipping scheduler and best checkpoint")
        ...
        if metric_is_finite and (args.main_metric_goal == "min" and metric_value < best_val_metric or
