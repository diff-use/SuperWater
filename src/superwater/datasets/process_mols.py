...
def parse_pdb_from_path(path):
    with warnings.catch_warnings():
        ...
        structure_rec = parse_structure(path, permissive=False, structure_id='random_id')
        structure_rec_lig = parse_structure(path, permissive=True, structure_id='random_id')
        rec = structure_rec[0]
...
def _water_mol2_block(coords):
    """TRIPOS mol2 text for a set of water oxygens, matching the waterbind ``_water.mol2``
    layout (one ``O.3`` atom per water, no bonds)."""
    lines = ["@<TRIPOS>MOLECULE", "water", f" {len(coords)} 0 0 0 0", "SMALL", "GASTEIGER", "", "@<TRIPOS>ATOM"]
    for i, (x, y, z) in enumerate(coords, 1):
        lines.append(f"{i:>7}  O        {x:9.4f}{y:9.4f}{z:9.4f} O.3   {i:<5} HOH{i:<8} 0.0000")
    lines.append("@<TRIPOS>BOND")
    return "\n".join(lines) + "\n"


def mol_from_water_pdb(pdb_path):
    """Build the water "ligand" RDKit mol from a ``*_water.pdb`` file.

    This replaces the old ``_water.mol2`` read so no .mol2 file (and no OpenBabel) is
    needed. The water-oxygen coordinates are read from the PDB with BioPython (robust,
    already a dependency), then parsed back through the EXACT RDKit mol2 sanitization the
    original code used (``MolFromMol2Block`` on an in-memory ``O.3`` block). RDKit derives
    the atom state (bond-less oxygen, formal charge -2, no implicit H) itself, so the
    per-atom featurization is bit-for-bit identical to the original ``_water.mol2`` path
    and existing checkpoints stay valid.
    """
    from superwater.utils.find_water_pos import find_real_water_pos
    coords = find_real_water_pos(pdb_path)
    if coords is None or len(coords) == 0:
        return None
    mol = Chem.MolFromMol2Block(_water_mol2_block(coords), sanitize=False, removeHs=False)
    if mol is None:
        return None
    try:
        Chem.SanitizeMol(mol)
        ...
        print(e)
        print(f"RDKit was unable to build waters from: {pdb_path}")
        return None
    return mol
