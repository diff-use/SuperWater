...
def loss_function(tr_pred, expand_tr_sigma, data, t_to_sigma, device, tr_weight=1, apply_mean=True):
    ...
    # before backward so unstable batches cannot corrupt model weights.
    ...
    for batch_idx, data in enumerate(tqdm(loader, total=num_batches)):
        ...
        try:
            ...
            if not torch.isfinite(loss):
                print(f'| WARNING: non-finite loss ({loss.item()}) at batch {batch_idx}, skipping batch')
            ...
            if not torch.isfinite(grad_norm):
                print(f'| WARNING: non-finite grad norm ({grad_norm}) at batch {batch_idx}, skipping optimizer step')
                optimizer.zero_grad(set_to_none=True)
                continue
